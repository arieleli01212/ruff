...
mod tests {
    ...
    fn predicate(id: u32) -> ScopedPredicateId {
        ScopedPredicateId::new(id as usize)
    }

    #[test]
    fn read_only_materialization_preserves_shared_state() {
        let mut constraints = ReachabilityConstraintsBuilder::default();
        let pending = PendingReachability::default();
        let mut state = PendingPlaceState::new(
            PlaceState::undefined(ScopedReachabilityConstraintId::ALWAYS_TRUE),
            pending.current,
        );
        let original = Rc::clone(&state.state);

        pending.materialize_ref(&mut state, pending.current, &mut constraints);

        assert!(Rc::ptr_eq(&state.state, &original));
    }

    #[test]
    fn complementary_pending_constraints_preserve_shared_state() {
        let mut constraints = ReachabilityConstraintsBuilder::default();
        let mut pending = PendingReachability::default();
        let root = pending.current;
        let truthy = constraints.add_atom(predicate(2));

        pending.push(truthy);
        let truthy_pending = pending.current;
        pending.current = root;
        pending.push(constraints.add_not_constraint(truthy));
        let falsy_pending = pending.current;

        let state = PendingPlaceState::new(
            PlaceState::undefined(ScopedReachabilityConstraintId::ALWAYS_TRUE),
            root,
        );
        let original = Rc::clone(&state.state);
        let mut current_states = IndexVec::<ScopedSymbolId, _>::from_iter([state.clone()]);
        let branch_states = IndexVec::<ScopedSymbolId, _>::from_iter([state]);
        let mut narrowing = NarrowingConstraintsBuilder::default();
        pending.current = truthy_pending;
        ...
        assert!(Rc::ptr_eq(
            &current_states[ScopedSymbolId::from_u32(0)].state,
            &original
        ));
        assert_eq!(
            current_states[ScopedSymbolId::from_u32(0)].reachability,
            truthy_pending
        );
    }

    #[test]
    fn non_complementary_pending_constraints_are_retained() {
        let mut constraints = ReachabilityConstraintsBuilder::default();
        let mut pending = PendingReachability::default();
        let root = pending.current;
        let first = constraints.add_atom(predicate(2));
        let second = constraints.add_atom(predicate(3));

        pending.push(first);
        let first_pending = pending.current;
        pending.current = root;
        pending.push(second);
        let second_pending = pending.current;
        ...
        let state = &current_states[ScopedSymbolId::from_u32(0)];
        assert!(!Rc::ptr_eq(&state.state, &original));
        assert_eq!(
            state
                .state
                ...
                .iter()
                .next()
                .expect("undefined state has an unbound binding")
                .reachability_constraint(),
            constraints.add_or_constraint(first, second)
        );
    }
}
