...
pub(crate) fn check(
    ...
    let notebook = query
        .as_notebook()
        .zip(source_kind.as_ipy_notebook().map(Notebook::index));
...
struct LspDiagnosticContext<'a> {
    ...
    notebook: Option<(&'a NotebookDocument, &'a NotebookIndex)>,
    supports_related_information: bool,
}
...
fn span_to_location(span: &Span, context: &LspDiagnosticContext) -> Option<lsp_types::Location> {
    ...
    if let Some((notebook, notebook_index)) = context.notebook {
