...
use super::{
    DescriptionLine, SectionBlock, SectionCandidate, SectionItem, SectionItemBuilder,
    is_uri_scheme_prefix, normalize_description, parse_named_items,
};
...
fn google_section_block(kind: SectionKind, body: &[ParsedLine<'_>]) -> Option<SectionBlock> {
    let items = if matches!(kind, SectionKind::Returns | SectionKind::Yields) {
...
fn split_google_return_type(line: &str) -> Option<(&str, &str)> {
    ...
    if is_docstring_type_expression(ty) {
        Some((ty, description))
    } else {
        None
    }
